...
import getNumberOption from '@hiram-labs/lrs-js-common/dist/config/getNumberOption';
import getStringOption from '@hiram-labs/lrs-js-common/dist/config/getStringOption';
import getDbFromUrl from '@hiram-labs/lrs-js-common/dist/mongoRepo/utils/getDbFromUrl';
...
const expressPort = getNumberOption(defaultTo(process.env.EXPRESS_PORT, process.env.PORT), DEFAULT_EXPRESS_PORT);
...
const mongoUrl = getStringOption(process.env.MONGO_URL, 'mongodb://localhost:27017/learninglocker_v2');
...
export default {
  ...
  s3StorageRepo: {
    awsConfig: {
      ...
      credentials: {
        ...
        secretAccessKey: getStringOption(process.env.FS_S3_SECRET_ACCESS_KEY, globalAwsIamAccessKeySecret)
      }
    } as S3ClientConfig,
    bucketName: getStringOption(process.env.FS_S3_BUCKET, 'xapi-service')
  },
  statementsService: {
    awaitUpdates: getBooleanOption(process.env.SERVICE_AWAIT_UPDATES, false),
    enableActorLowerCasing: getBooleanOption(process.env.STATEMENTS_SERVICE_LOWERCASE_ACTORS, false),
  ...
  winston: {
    cloudWatch: {
      awsConfig: {
        accessKeyId: getStringOption(process.env.WINSTON_CLOUDWATCH_ACCESS_KEY_ID, globalAwsIamAccessKeyId),
        region: getStringOption(process.env.WINSTON_CLOUDWATCH_REGION, globalAwsRegion),
        secretAccessKey: getStringOption(process.env.WINSTON_CLOUDWATCH_SECRET_ACCESS_KEY, globalAwsIamAccessKeySecret)
      },
