...
describe('expressPresenter.deleteProfile with scopes', () => {
  ...
  it('should throw forbidden error when using invalid scope', async () => {
    await deleteProfile().set('Authorization', TEST_INVALID_SCOPE_TOKEN).expect(StatusCodes.FORBIDDEN);
  });

  it('should throw forbidden error when using expired client', async () => {
    await deleteProfile().set('Authorization', TEST_EXPIRED_ORG_TOKEN).expect(StatusCodes.FORBIDDEN);
  });
  ...
  it('should not error when using valid scopes', async () => {
    await deleteProfile().set('Authorization', TEST_VALID_SCOPE_TOKEN).expect(StatusCodes.NO_CONTENT);
  });
});
