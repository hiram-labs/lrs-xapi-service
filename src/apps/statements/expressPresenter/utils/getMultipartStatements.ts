...
export default async (req: Request) => {
  ...
  const hasStatements =
    parts.length >= 1 && jsonContentTypePattern.test(get<any, string, string>(parts[0].headers, 'content-type', ''));
  ...
  const attachments = parts.slice(1).map((part): AttachmentModel => {
    const contentTransferEncoding = get(part.headers, 'content-transfer-encoding') as string | undefined;
