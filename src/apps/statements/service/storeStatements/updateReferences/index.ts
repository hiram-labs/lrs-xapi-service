...
export default async (config: Config, models: UnstoredStatementModel[], client: ClientModel) => {
  ...
  const traverseUp = async (visitedIds: string[], refIds: string[], modelId: string): Promise<string[]> => {
  ...
  const traverseUpRefs = async (visitedIds: string[], refIds: string[], upRefIds: string[]): Promise<string[]> => {
  ...
  await models.reduce(
    async (results, model): Promise<string[]> => {
