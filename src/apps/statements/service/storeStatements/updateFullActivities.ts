...
export default async ({ config, models, client }: Opts): Promise<void> => {
  ...
  const fullActivities = map(groupedActivities, (matchingActivities, activityId): FullActivityDatabase => {
    ...
    const mergedContextActivities = contextActivitiesForFullActivity.reduce((acc, contextActivity) => {
    ...
    const type = last(
      matchingActivities.map((matchingActivity) => matchingActivity.type).filter((value) => value !== undefined)
    );

    const moreInfo = last(
      matchingActivities.map((matchingActivity) => matchingActivity.moreInfo).filter((value) => value !== undefined)
    );
    ...
    return {
      ...
      ...(isEmpty(mergedContextActivities) ? {} : { context: { contextActivities: mergedContextActivities } }),
      ...(type !== undefined ? { type } : {}),
