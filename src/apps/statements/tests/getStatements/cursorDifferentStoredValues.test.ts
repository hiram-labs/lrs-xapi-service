...
describe('get statements with different stored value using cursor', () => {
  const service = setup();
  const createStatementPartial = (documentId: string, statementId: string, stored: string): Document => ({
  ...
  it('should return correct statements when ascending', async () => {
    ...
    await db.collection('statements').insertMany([statement1, statement2, statement3, statement4, statement5]);
