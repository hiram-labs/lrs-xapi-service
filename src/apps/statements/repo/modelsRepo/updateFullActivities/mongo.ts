...
const createBatchQuery = (batch: UnorderedBulkOperation) => (fullActivity: FullActivityDatabase) => {
  ...
  batch
    ...
    .upsert()
    .updateOne({
      $set: !isEmpty(mongoSet) ? mongoSet : {}
      // $addToSet: !isEmpty(mongoAddToSet) ? mongoAddToSet : {}
    });
};
...
export default (config: FacadeConfig): Signature =>
  async ({ fullActivities }) => {
    ...
    fullActivities.forEach(createBatchQuery(batch));
    await batch.execute();
  };
