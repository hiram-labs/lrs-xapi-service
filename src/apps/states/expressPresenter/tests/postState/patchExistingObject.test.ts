...
describe('expressPresenter.postState with existing object content', () => {
  ...
  it('should merge when patching without registration', async () => {
    await createObjectState();
    await patchState({ registration: undefined }, TEST_OBJECT_PATCH_CONTENT, JSON_CONTENT_TYPE).expect(
  ...
  it('should merge when patching with mbox_sha1sum', async () => {
    ...
    await patchState({ agent: JSON.stringify(TEST_MBOXSHA1_AGENT) }, TEST_OBJECT_PATCH_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
  ...
  it('should merge when patching with openid', async () => {
    ...
    await patchState({ agent: JSON.stringify(TEST_OPENID_AGENT) }, TEST_OBJECT_PATCH_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
  ...
  it('should merge when patching with account', async () => {
    ...
    await patchState({ agent: JSON.stringify(TEST_ACCOUNT_AGENT) }, TEST_OBJECT_PATCH_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
