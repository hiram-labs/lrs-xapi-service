...
import { TEST_ACCOUNT_AGENT, TEST_MBOX_AGENT, TEST_MBOXSHA1_AGENT, TEST_OPENID_AGENT } from '../../../utils/testValues';
...
describe('expressPresenter.deleteStates with existing state', () => {
  ...
  it('should delete when deleting with an mbox_sha1sum', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_MBOXSHA1_AGENT) }).expect(StatusCodes.NO_CONTENT);
  ...
  it('should delete when deleting with an account', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_ACCOUNT_AGENT) }).expect(StatusCodes.NO_CONTENT);
