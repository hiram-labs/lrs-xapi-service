...
describe('expressPresenter.putState with existing model', () => {
  ...
  it('should overwrite model when overwriting existing JSON', async () => {
    ...
    await overwriteState({}, TEST_OBJECT_PATCH_CONTENT, JSON_CONTENT_TYPE).expect(StatusCodes.NO_CONTENT);
    await assertState(TEST_OBJECT_PATCH_CONTENT);
  });

  it('should overwrite model when overwriting without a registration', async () => {
    await createTextState();
    await overwriteState({ registration: undefined }, TEST_IMMUTABLE_CONTENT).expect(StatusCodes.NO_CONTENT);
    await assertState(TEST_IMMUTABLE_CONTENT);
  });
  ...
  it('should overwrite model when overwriting with mbox_sha1sum', async () => {
    ...
    await overwriteState({ agent: JSON.stringify(TEST_MBOXSHA1_AGENT) }, TEST_IMMUTABLE_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
  ...
  it('should overwrite model when overwriting with openid', async () => {
    ...
    await overwriteState({ agent: JSON.stringify(TEST_OPENID_AGENT) }, TEST_IMMUTABLE_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
  ...
  it('should overwrite model when overwriting with account', async () => {
    ...
    await overwriteState({ agent: JSON.stringify(TEST_ACCOUNT_AGENT) }, TEST_IMMUTABLE_CONTENT).expect(
      StatusCodes.NO_CONTENT
    );
