...
import { TEST_CONTENT, TEST_OUTSIDE_ORG_TOKEN, TEST_OUTSIDE_STORE_TOKEN } from '../../../utils/testValues';
...
describe('expressPresenter.putProfile when outside client', () => {
  ...
  const overwriteOutsideProfile = async (token: string) => {
    await overwriteProfile({}, 'unused_content').set('Authorization', token).expect(StatusCodes.NO_CONTENT);
  };
