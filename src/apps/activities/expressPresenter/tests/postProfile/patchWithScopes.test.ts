...
describe('expressPresenter.postProfile with scopes', () => {
  ...
  it('should throw forbidden error when using invalid scope', async () => {
    await patchProfile().set('Authorization', TEST_INVALID_SCOPE_TOKEN).expect(StatusCodes.FORBIDDEN);
  });
  ...
  it('should not throw an error when using valid scopes', async () => {
    await patchProfile().set('Authorization', TEST_VALID_SCOPE_TOKEN).expect(StatusCodes.NO_CONTENT);
  });
});
